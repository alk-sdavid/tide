...
eval "function _tide_pwd
    if set -l split_pwd (string replace -r '^$HOME' '~' -- \$PWD | string replace -r '^~/sources/' '/' | string split /)
    ...
    i=1 for dir_section in \$split_pwd[2..-2]
        string join -- / \$split_pwd[..\$i] | string replace '/' '~/sources/' | string replace '~' $HOME | read -l parent_dir # Uses i before increment
